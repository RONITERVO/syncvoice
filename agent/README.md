...
By default, published cues preserve the timing observed as Gemini's output-transcription fragments arrive. This slightly irregular timing can feel like live generation. Deterministic full-duration pacing is an explicit opt-in: enable **Normalize transcript timing** in the Production agent, or add `--normalize-cues` to `agent:generate`. Existing generated libraries can also be normalized without touching audio:

```powershell
node agent/normalize-cues.mjs --manifest D:\game\.syncvoice\project.json --asset-root D:\game-audio\assets\syncvoice --locale en-US,fi-FI
```
